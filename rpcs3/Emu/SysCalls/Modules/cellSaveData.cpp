	...
	PFuncFile funcFile, u32 container, u32 unknown, vm::ptr<void> userdata, u32 userId, PFuncDone funcDone)
{
	...
	// Get save stats
	{
		...
		if (statSet->setParam)
		{
			...
			psf =
			{
				{ "ACCOUNT_ID", { psf::format::array, 16 } }, // ???
				{ "ATTRIBUTE", psf::entry{ statSet->setParam->attribute } },
				{ "CATEGORY",  { psf::format::string, 2, "SD" } }, // ???
				{ "PARAMS", { psf::format::string, 16 } }, // ???
				{ "PARAMS2", { psf::format::string, 16 } }, // ???
				{ "PARENTAL_LEVEL", psf::entry{ 0 } }, // ???
				{ "DETAIL", { psf::format::string, 1024, statSet->setParam->detail } },
				{ "SAVEDATA_DIRECTORY", { psf::format::string, 256, save_entry.dirName } },
				{ "SAVEDATA_LIST_PARAM", { psf::format::string, 8, statSet->setParam->listParam } },
				{ "SUB_TITLE", { psf::format::string, 128, statSet->setParam->subTitle } },
				{ "TITLE", { psf::format::string, 128, statSet->setParam->title } },
			};
		}
		else if (psf.empty())
		{
